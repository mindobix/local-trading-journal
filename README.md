...
Now includes a **Signal Intel** tab — a live market intelligence feed with AI-powered signal analysis, real-time stock prices, and RSS news aggregation running on a local background server — plus a **New Signals** aggregated feed, an **LLM Prompts** tab for saving and launching AI market analysis prompts, an **LLM Trade Plan** tab for creating and managing option trade plan cards generated by any LLM, and a **Banking** tab for tracking brokerage account funding and transfers.
...
| **Market intelligence** | Signal Intel — AI analysis, live prices | Rare / paywalled |
...
### Banking Tab
Track brokerage account funding, withdrawals, and transfers in one place — fully offline, stored in IndexedDB.

**Brokerage grids** — each brokerage gets its own section with a date/bank/brokerage entry grid:
- **Inline add and edit** — click any cell to edit; no dialogs or popups
- **Auto-populate opposite amount** — enter a bank amount and the brokerage column auto-fills with the inverse (e.g., bank +$1,000 → brokerage −$1,000)
- **Currency formatting** — amounts display as `$1,234.56` while not focused; raw number while editing
- **Sort per brokerage** — click the sort icon on any column header to toggle asc/desc; preference saved to IndexedDB
- **Collapse / expand brokerage** — click the chevron next to a brokerage name to collapse it; collapsed state saved to IndexedDB; collapsed rows show a subtotal strip with bank and brokerage sums at a glance
- **Year grouping** — entries within each brokerage are grouped by year, most-recent year first; non-current years start collapsed and show inline subtotals when collapsed; click any year header to toggle
- **Drag to reorder** — grab the drag handle on any brokerage header to reorder brokerages; new order saved to IndexedDB
- **Duplicate Add Row** — each brokerage has its own "+ Add Row" button below the subtotal so you never need to scroll to the top

**Pivot summary table** — a single *By Year / Month* hierarchical table displayed alongside the grids:
- Each year is a collapsible row showing annual totals across all bank and brokerage columns
- The current year is expanded by default; all prior years start collapsed
- Expanding a year reveals month-by-month sub-rows sorted latest month first
- Overall Totals card appears above the pivot for quick reference

**Manage Accounts** — inline account editor accessible via the ⚙ button: add, rename, or delete accounts; drag to reorder within a brokerage. No dialogs.

**Filter bar hidden** — the global filter bar and stats bar are hidden when Banking is the active tab.
...
- Add, edit, or remove RSS feeds per ticker from the **Signal Intel Settings** panel
...
Then open `http://localhost:3737` in your browser. The Signal Intel tab will connect automatically.
...
| `bk-sort-state` | Sort direction per brokerage (Banking tab) | No |
| `bk-collapse-state` | Collapse state per brokerage (Banking tab) | No |
...
│   ├── news.js             # Signal Intel + LLM Prompts tabs — UI, polling, report panel, prices, prompt manager
...
### Signal Intel server (`news-crawler/`)
| Package | Purpose |
|---------|---------|
| `express` | HTTP server |
